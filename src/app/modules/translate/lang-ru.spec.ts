...
describe('TranslateService dictionary: lang-ru', () => {
  it('should have translations defined', () => {
    ...
    expect(LANG_RU_TRANSLATIONS).toEqual(
      jasmine.objectContaining({
        ...
        services: {
          ...
          paragraph1:
            'Kos.Mos.Music.Lab это "дочка" Московского независимого лейбла Kos.Mos.Music, одним из владельцев которого является Андрей Буртаев, более известный под псевдонимом Electrosoul System. C момента стновленя цифровых форматов как популярной формы распространения музыкальной продукции, команда лейбла приняла решение делать мастеринг самостоятельно, поскольку результаты выполнения этой работы другими людьми не всегда удовлетворяли, что выливалось в задержки релизов. Так появилась Kos.Mos.Music.Lab.',
          paragraph2:
            'На данный момент, накопив солидный опыт и поработав с такими продюсерами как Subwave, Future Engineers, Roygreen & Protone и Electrosoul System (да, Андрей не занимается мастерингом собственных треков) команда Kos.Mos.Music.Lab рада предоставить вам свои услуги по мастерингу и сведению ваших музыкальных творений. Мастеринг почти всех релизов лейбла Kos.Mos.Music, начиная с 11-го, был сделан нами, а треки лейбла играет в своих сетах и подкастах такой неоспоримый драм-н-бейс гуру как London Elektricity, а также другие продюсеры и диск-жокеи лейблов Hospital, Med School и не только.',
          paragraph3:
            'Индивидуальный подход к каждой композиции это наш способ достижения наилучших результатов.',
          requirements:
            'Технические требования: 320 mp3/kbps, полная версия, формат наименования файла: псевдоним_артиста - название_трека (эл_почта@domain.tld).',
        },
        demo: {
          open: 'Отправить демо',
          title: 'Отправка демо',

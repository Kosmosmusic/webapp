...
if [ 1 -gt $# ]; then
  ...
  # prompt user for variables one by one: FIREBASE_APP_ID
  printf "${LIGHT_BLUE}  >> step ${YELLOW}9/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}FIREBASE_APP_ID${LIGHT_BLUE} ${DEFAULT} \n"
  ...
  printf "${LIGHT_BLUE}  >> step ${YELLOW}12/${totalSteps} ${LIGHT_BLUE}: You provided the following values:\n
    ...
    - ${YELLOW}FIREBASE_APP_ID${LIGHT_BLUE}=${LIGHT_GREEN}${privilegedAccessFirebaseUid}${LIGHT_BLUE}\n
  ...
  y | Y)
    ...
    echo "FIREBASE_APP_ID=${privilegedAccessFirebaseUid}" >>./.env
  ...
  *)
  ...
  # prompt user for variables one by one: MAILER_RECIPIENT_EMAIL_CONTACT
  printf "${LIGHT_BLUE}  >> step ${YELLOW}22/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_RECIPIENT_EMAIL_CONTACT${LIGHT_BLUE} ${DEFAULT} \n"
  read -p "   > input value :" mailerRecipientEmailContact

  # prompt user for variables one by one: MAILER_RECIPIENT_EMAIL_DEMO
  printf "${LIGHT_BLUE}  >> step ${YELLOW}23/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_RECIPIENT_EMAIL_DEMO${LIGHT_BLUE} ${DEFAULT} \n"
  read -p "   > input value :" mailerRecipientEmailDemo

  # prompt user for variables one by one: MAILER_RECIPIENT_EMAIL_BOOKING
  printf "${LIGHT_BLUE}  >> step ${YELLOW}24/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_RECIPIENT_EMAIL_BOOKING${LIGHT_BLUE} ${DEFAULT} \n"
  read -p "   > input value :" mailerRecipientEmailBooking

  # prompt user for variables one by one: MAILER_RECIPIENT_EMAIL_MASTERING
  printf "${LIGHT_BLUE}  >> step ${YELLOW}25/${totalSteps} ${LIGHT_BLUE}: Enter ${YELLOW}MAILER_RECIPIENT_EMAIL_MASTERING${LIGHT_BLUE} ${DEFAULT} \n"
  read -p "   > input value :" mailerRecipientEmailMastering

  # summary check
  printf "${LIGHT_BLUE}  >> step ${YELLOW}26/${totalSteps} ${LIGHT_BLUE}: You provided the following values:\n
    ...
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_CONTACT${LIGHT_BLUE}=${LIGHT_GREEN}${mailerRecipientEmailContact}${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_DEMO${LIGHT_BLUE}=${LIGHT_GREEN}${mailerRecipientEmailDemo}${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_BOOKING${LIGHT_BLUE}=${LIGHT_GREEN}${mailerRecipientEmailBooking}${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_MASTERING${LIGHT_BLUE}=${LIGHT_GREEN}${mailerRecipientEmailMastering}${LIGHT_BLUE}\n"

  # notify user
  printf "${LIGHT_BLUE}  >> step ${YELLOW}27/${totalSteps} ${LIGHT_BLUE}: Compare changes with existing ${YELLOW}./functions/.env${LIGHT_BLUE} file${DEFAULT} \n\n"
  cat ./functions/.env

  # prompt user whether to continue or not
  printf "\n${LIGHT_BLUE}  >> step ${YELLOW}28/${totalSteps} ${LIGHT_BLUE}: Continue and create/overwrite ${YELLOW}.env${LIGHT_BLUE} file${DEFAULT} \n"
  read -p "   > continue (y/n) :" userChoice
  case $userChoice in
  y | Y)
    # write file contents
    ...
    echo "MAILER_RECIPIENT_EMAIL_CONTACT=${mailerRecipientEmailContact}" >>./functions/.env
    echo "MAILER_RECIPIENT_EMAIL_DEMO=${mailerRecipientEmailDemo}" >>./functions/.env
    echo "MAILER_RECIPIENT_EMAIL_BOOKING=${mailerRecipientEmailBooking}" >>./functions/.env
    echo "MAILER_RECIPIENT_EMAIL_MASTERING=${mailerRecipientEmailMastering}" >>./functions/.env
    # notify user
  ...
  *)
  ...
  mailerRecipientEmailContact=${19}
  mailerRecipientEmailDemo=${20}
  mailerRecipientEmailBooking=${21}
  mailerRecipientEmailMastering=${22}

  # summary check
  printf "${LIGHT_BLUE} >> You provided the following values:\n
    # client variables:\n
    - ${YELLOW}SOUNDCLOUD_CLIENT_ID${LIGHT_BLUE}=${LIGHT_GREEN}${soundcloudUserId}${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_API_KEY${LIGHT_BLUE}=${LIGHT_GREEN}${firebaseApiKey}${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_AUTH_DOMAIN${LIGHT_BLUE}=${LIGHT_GREEN}${firebaseAuthDomain}${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_DATABASE_URL${LIGHT_BLUE}=${LIGHT_GREEN}${firebaseDatabaseUrl}${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_PROJECT_ID${LIGHT_BLUE}=${LIGHT_GREEN}${firebaseProjectId}${LIGHT_BLUE}\n
  ...
  printf "${LIGHT_RED} >> ERROR: ${LIGHT_BLUE}you should provide either no arguments at all or 22 for the following keys in this partucular sequence (latter  case is not safe, env files will be rewritten without prompting for confirmation, it's intended for CI usage):\n
    - ${YELLOW}SOUNDCLOUD_CLIENT_ID${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_API_KEY${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_AUTH_DOMAIN${LIGHT_BLUE}\n
    - ${YELLOW}FIREBASE_DATABASE_URL${LIGHT_BLUE}\n
    ...
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_CONTACT${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_DEMO${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_BOOKING${LIGHT_BLUE}\n
    - ${YELLOW}MAILER_RECIPIENT_EMAIL_MASTERING${LIGHT_BLUE}\n\n"
fi

...
if (process.argv[2] === 'prod') {
   ...
   * user if user requests kosmosmusic.ru/index by typing it in browser's address bar.
   */
  app.use((req, res) => {
    res.sendFile(__dirname + '/dist/kosmosmusic/index.html');
  });
}

server.listen(process.env.PORT || 3000, process.env.IP || '0.0.0.0', () => {
  const addr = server.address();
  console.log(`Kosmosmusic dev server listening at ${addr.address}:${addr.port}`);
});
